...
class ExpiryDateFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    // أول رقمين = شهر → بعدها أضيف "/"
    if (text.length == 2 && !text.contains('/')) {
      text = text + '/';
    }

    // لو رجع المستخدم ومسح → نتأكد ما يبقى "/" وحيد
...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> _placeOrder() async {
    ...
    try {
      ...
      List<String> prescriptionUrls = []; // ✅ Collect prescription links

      for (var doc in ordersSnapshot.docs) {
        final data = doc.data() as Map<String, dynamic>;
        totalAmount += (data['price'] ?? 0) * (data['quantity'] ?? 1);

        // ✅ جمع الوصفات من اللستة فقط
        ...
        // Save item inside placedOrder.items
        await placedOrderRef.collection("items").add(data);

        // Remove from cart (orders)
      ...
      if (hasPrescription) {
        orderStatus = "pending"; // الوصفة محتاجة موافقة

        if (_paymentMethod == "card") {
          paymentStatus = "authorized"; // مقيد ينتظر موافقة
        } else {
          paymentStatus = "pending_cash"; // كاش لكن الوصفة معلقة
        }
      } else {
        orderStatus = "approved"; // ما يحتاج وصفة
        if (_paymentMethod == "card") {
          paymentStatus = "captured"; // يسحب مباشرة
        } else {
          paymentStatus = "pending_cash"; // كاش عادي
        }
      }


      // ✅ Save placed order data
      await placedOrderRef.set({
        ...
        "prescriptions": prescriptionUrls, // ✅ هنا فقط
      });
              ...
              Row(children: [
                Expanded(
                  child: TextFormField(
                    ...
                    inputFormatters: [
                      FilteringTextInputFormatter.allow(RegExp(r'[0-9/]')), // ✅ أرقام و "/" مسموح
  ...
  Widget _buildDeliverySlot(String slot, Color textColor) {
    ...
    return RadioListTile<String>(
      title: Text(
        slot,
        style: TextStyle(
          color: isPast ? Colors.grey : textColor, // ✅ يبين حسب الثيم
        ),
      ),
      value: slot,
  ...
  Widget _buildTextField(TextEditingController c, String hint, Color textColor,
      Color? cardColor, TextInputType type,
